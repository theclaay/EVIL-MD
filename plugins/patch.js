  ...
  async (message, input) => {
    try {
      ...
      if (
        ...
        menuThemeHeader = "☇ ❀̥ " + Config.botname + "❀̥";
        menuThemeCommandPrefix = "│ │";
        menuThemeFooter = "╰═══════════════⊷";
        menuThemeCategoryHeader = "╭─❍";
        menuThemeCategoryFooter = "══⊷❍";
      ...
      const designs = [
        async () => {
          const imageBuffer = await axios.get(
            "https://telegra.ph/file/034990b2d62595148af2e.jpgImages/logo.jpg",
            {
...
smd(
  ...
  async (m) => {
    try {
      ...
      const message = `*EVIL-MD Running Since:* ${uptimeHours}h ${uptimeMinutes}ᴍ ${uptimeSeconds}s\n \t_ʜᴇʀᴇ's ᴍᴏʀᴇ ɪɴғᴏ_\n*ʀᴀᴍ ᴜsᴀɢᴇ:* ${ramUsage.toFixed(2)} MB\n*ᴄᴘᴜ ɴᴀᴍᴇ:* ${cpuModel}`;
      const contextInfo = {
        isForwarded: true,
        forwardingScore: 999,
        title: "EVIL Running Since",
        body: message,
        footerText: "EVIL || Dark Evil",
